...
public class OpponentMobilityGamer extends StateMachineGamer {
	...
	private int maxScore(Role role, MachineState state, int level, int limit) throws GoalDefinitionException {
		try {
			...
			if ( level >= limit ) {
				// Still only return self mobility if single-player game.
				if ( stateMachine.getRoles().size() == 1 ) {
					return evalHeuristic(role,state);
				}
				return evalOppHeuristic(role,state);
			}
			List<Move> actions = stateMachine.getLegalMoves(state, role);
			int score = 0;
			for (int i = 0; i < actions.size(); i++) {
	...
	// For now, opponent mobility heuristic
	private int evalOppHeuristic(Role role, MachineState state) throws GoalDefinitionException {
		try {
			StateMachine stateMachine = getStateMachine();
			List<Move> myActions = stateMachine.getLegalMoves(state,role);
			int numActions = 0;
			for (int i=0; i<myActions.size(); i++) {
				List<List<Move>> oppActions = stateMachine.getLegalJointMoves(state,role,myActions.get(i));
				// Select set where opponent has greatest mobility
				if (oppActions.size() > numActions) {
					numActions = oppActions.size();
				}
			}

			List<Role> roles = stateMachine.getRoles();
			ArrayList<Role> oppRoles = new ArrayList<Role>(roles);
			oppRoles.remove(getRole());

			// Sum all opponent feasible moves
			int numFeasibles = 0;
			for (int i=0; i<oppRoles.size(); i++) {
				List<Move> feasibles = stateMachine.findActions(oppRoles.get(i));
				numFeasibles = numFeasibles + feasibles.size();
			}
			return ((numActions * 100) / numFeasibles );
		}
		catch (Exception e) {
			throw new GoalDefinitionException(state,role);
		}
	...
	public String getName() {
		return "Opponent Mobility Player";
	}
